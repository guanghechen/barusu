...
const exec = (content: string): { lastIndex: number, groups: Record<string, string> } | null => {
...
runCaseTree(
  path.resolve(caseDir, 'module-statement.json'),
  kase => {
    const result = exec(kase.input as string)
    expect({ input: kase.input as string, result }).toMatchSnapshot('execWithMultipleRegex')
    ...
    for (const semicolon of [false, true]) {
      for (const indent of ['  ', '\t']) {
        ...
        const formattedString: string = formatStaticModuleStatementItem(
          statementItem, quote, indent, semicolon, 80)
