...
export const createMainCommand: MainCommandCreator<MainCommandOptions> = function (
  ...
  command
    ...
    .option(
      ...
      'blank lines after import/export statement',
    )
    .action(async function ([_workspaceDir], options: MainCommandOptions) {
      ...
      logger.debug(
        'indent:',
        indent.replace(/[ ]/g, '_').replace(/[\t]/g, '\\t'),
      )

      // resolve maxColumn
      const maxColumn: number = coverNumber(
        defaultOptions.maxColumn,
        options.maxColumn,
      )
      ...
      if (moduleRanks != null) {
        ...
        for (let i = 0; i < moduleRanks.length; ++i) {
          ...
          if (
            ...
            throw new Error(
              `[Bad option] missed regex / rank in moduleRanks[${i}]: ${JSON.stringify(
                item,
              )}`,
            )
          }
