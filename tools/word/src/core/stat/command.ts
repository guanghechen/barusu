...
export type SubCommandStatOptions = SubCommandOptions &
  CommandConfigurationFlatOpts

/**
 * create Sub-command: stat (s)
 */
export const createSubCommandStat: SubCommandCreator<SubCommandStatOptions> = function (
  ...
  command
    ...
    .option(
      ...
      'source file path using to give statistics',
      (val, acc: string[]) => acc.concat(val),
      [],
    )
    .option(
      '-p, --file-pattern <filePattern>',
      'file wildcard list using to give statistics',
      (val, acc: string[]) => acc.concat(val),
      [],
    )
    .option(
      '--show-details <lineNumber>',
      'rows in the word frequency ranking list to be displayed',
    )
    .option(
      '--show-details-pretty',
      'Filter out blank and punctuation characters & set --show-details default to 10',
    )
    ...
    .action(async function ([_workspaceDir], options: SubCommandStatOptions) {
      ...
      const showDetailsPretty: boolean = coverBoolean(
        defaultOptions.showDetailsPretty,
        options.showDetailsPretty,
      )
      logger.debug('showDetailsPretty:', showDetailsPretty)

      // resolve showDetails
      const showDetails: number = coverNumber(
        showDetailsPretty ? 10 : defaultOptions.showDetails,
        options.showDetails,
      )
      ...
      const showSummaryOnly: boolean = coverBoolean(
        defaultOptions.showSummaryOnly,
