...
export function serveDataFile(
  { workspaceDir, prefixUrl, mockDataRootDir }: Params
): Koa.Middleware {
  logger.info(`[serveDataFile]: mount ${ prefixUrl } --> ${ relativeOfWorkspace(workspaceDir, mockDataRootDir) }`)
      ...
      const resolvedFilepath: string | null = (() => {
        ...
        for (const p of potentialDataPaths) {
          ...
          if (fs.existsSync(absoluteFilepath)) {
            logger.debug(`[serveDataFile hit] try filepath: ${ relativeFilepath }`)
            return p
          }
