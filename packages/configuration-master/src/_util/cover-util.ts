 ...
 * @param value         传进来的值
 */
export type CoverOperationFunc<T> = (
  defaultValue?: T,
  value?: any,
) => CoverOperationResult<T>
 ...
 */
export const coverNumber: CoverOperationFunc<number> = (
  defaultValue,
  value,
) => {
  ...
  if (v == null || Number.isNaN(v)) {
    return result.addError(
      `(${stringify(value)}) is not a valid number (or number string).`,
    )
  }
 ...
 */
export const coverInteger: CoverOperationFunc<number> = (
  defaultValue,
  value,
) => {
  const result: CoverOperationResult<number> = new CoverOperationResult(
  ...
  if (v == null || Number.isNaN(v)) {
    return result.addError(
      `(${stringify(value)}) is not a valid integer (or integer string)`,
    )
  }
 ...
 * 否则，若不是布尔值，视作类型错误
 */
export const coverBoolean: CoverOperationFunc<boolean> = (
  defaultValue,
  value,
) => {
  ...
  switch (value) {
    case 'false':
    case false:
      return result.setValue(false)
    case 'true':
    case true:
      return result.setValue(true)
    default:
      return result.addError(
        `(${stringify(value)}) is not a valid boolean type (or boolean string)`,
      )
  }
}
...
export const coverNull: CoverOperationFunc<null> = (defaultValue, value) => {
  const result: CoverOperationResult<null> = new CoverOperationResult(
    defaultValue,
  )
  if (value === undefined) return result
 ...
 */
export const coverString: CoverOperationFunc<string> = (
  defaultValue,
  value,
) => {
...
export const coverRegex: CoverOperationFunc<RegExp> = (defaultValue, value) => {
  ...
  try {
    ...
    result.addError(
      error.message || `(${stringify(value)}) is not a valid regex.`,
    )
  }
 ...
 */
export function coverArray<T>(
  elemCoverFunc: CoverOperationFunc<T>,
): CoverOperationFunc<T[]> {
