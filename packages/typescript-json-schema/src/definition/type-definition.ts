...
export function getTypeDefinition(
  ...
  while (
    ...
    (type.aliasSymbol.escapedName === 'Readonly' ||
      type.aliasSymbol.escapedName === 'Mutable') &&
  ...
  const isRawType =
    ...
    (context.checker.getFullyQualifiedName(symbol) !== 'Window' &&
      (context.checker.getFullyQualifiedName(symbol) === 'Date' ||
        symbol.name === 'integer' ||
        context.checker.getIndexInfoOfType(type, ts.IndexKind.Number) !==
          undefined))
  ...
  if (type.flags & ts.TypeFlags.Union) {
    ...
    isStringEnum = unionType.types.every(
      propType => (propType.getFlags() & ts.TypeFlags.StringLiteral) !== 0,
    )
  }

  // aliased types must be handled slightly different
  const asTypeAliasRef =
    asRef && reffedType && (context.args.aliasRef || isStringEnum)
  if (!asTypeAliasRef) {
  ...
  if (asTypeAliasRef) {
    ...
    if (context.args.uniqueNames && type.symbol) {
      ...
      const relativePath = path.relative(process.cwd(), sourceFile.fileName)
      fullTypeName = `${context.getTypeName(type)}.${generateHashOfNode(
        getCanonicalDeclaration(tSymbol),
        relativePath,
      )}`
    } else if (
      reffedType &&
      context.hasSchemaOverride(reffedType.escapedName as string)
    ) {
  ...
  if (!asRef || !context.hasReffedDefinition(fullTypeName)) {
    if (asRef) {
      ...
      const reffedDefinition: Definition =
        asTypeAliasRef && reffedType && type.symbol !== reffedType && symbol
...
export function getDefinitionForRootType(
  ...
  if (tupleType) {
    ...
    const fixedTypes = elemTypes.map(elType =>
      getTypeDefinition(context, elType as any),
    )
    // eslint-disable-next-line no-param-reassign
      ...
      if (value !== undefined) {
        ...
        if (
          ...
          definition.patternProperties = {
            [context.NUMERIC_INDEX_PATTERN]: getTypeDefinition(
              context,
              arrayType,
            ),
          }
