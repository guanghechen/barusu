...
import { regexRequire } from '../src/util'
...
const getValues = (singleQuotation: boolean): QuoteInfo => {
...
const matchSimple = (
  ...
  assert(
    ...
    `File doesn't match, got: ${match![2]}, expected: ${expectedFileName}`,
  )
  assert(
    match![4] === propertyName,
    `Property has to be ${propertyName?.toString()}`,
  )
}

const commonTests = (singleQuotation: boolean): void => {
...
const tests = (singleQuotation: boolean, objectName?: string): void => {
